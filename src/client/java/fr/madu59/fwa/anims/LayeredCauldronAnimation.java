    ...
    private void renderFilteredQuads(PoseStack poseStack, VertexConsumer buffer, List<BakedQuad> quads, boolean wantLiquid, int light) {
        for (BakedQuad quad : quads) {
            ...
            if ((path.contains("cauldron") && !last.contains("liquid") && !last.contains("water")) != wantLiquid){
                ...
                if (quad.isTinted()) {
                    int color = Minecraft.getInstance().getBlockColors().getColor(newBlockState, Minecraft.getInstance().level, position, quad.tintIndex());
                    if(color != -1){
                ...
                RenderHelper.renderQuad(buffer, poseStack.last(), quad, 1.0f, r, g, b, light, getAnimationBlockData());
            }
        }
    }
}
